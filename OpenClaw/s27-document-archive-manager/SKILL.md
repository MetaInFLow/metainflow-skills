...
- `feishu_skill_plan.json`
  - 可执行的飞书技能调用计划
  - 对写表动作必须先包含 `resolve_target_tables_and_schema`
...
- 涉及 `feishu-bitable` 写入时，必须先读取当前目标表结构，确认目标表、主键字段和必填字段后再执行写入。
- 对真实飞书表的字段映射必须以当前表结构为准；字段不存在、字段类型不兼容或仅有本地路径而无有效 URL 时，不得继续写入该字段。
